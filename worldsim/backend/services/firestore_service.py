...
def write_event(event_dict: dict) -> bool:
    """
    Write complete event dictionary to Firestore events collection.

    Accepts full enriched event dict directly. Adds timestamp automatically.
    No filtering of fields — all nested objects (sender_before, trust_after, etc.)
    are written as-is.

    Args:
        event_dict: Complete event dictionary with all enriched fields.
